...
export const CardDesignSelector: React.FC<any> = ({ path }) => {
  ...
  return (
    <div className="flex flex-col gap-4 py-4">
      <div className="text-xl-semi text-primary mb-2">Select Product Card Design</div>
        ...
        {options.map((opt) => (
          ...
          >
            <img
              src={opt.image}
              alt={opt.label}
              className="w-full h-auto aspect-3/4 rounded-soft mb-2 object-contain"
            />
            <div className="text-base-semi">{opt.label}</div>
          </div>
        ))}
      </div>
    </div>
